...
class ImportChartAccount(models.TransientModel):
    ...
    import_option = fields.Selection([('csv', 'CSV File'), ('xls', 'XLS File'),('pdf','PDF File')], string='Select', default='csv')

    def import_file(self):

        if self.import_option == 'csv':

            keys = ['first_name', 'last_name', 'date_ask', 'date_response', 'company']
    ...
    def create_chart(self, values):
        ...
        service_obj = self.env['convention.convention']

        data = {
            'first_name': values.get('first_name'),
            'date_ask': values.get('date_ask'),
            'date_response': values.get('date_response')
        }
